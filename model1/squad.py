...
class data_generator:
    ...
    def __iter__(self):
        while True:
            ...
            X1, X2, S1, S2 = [], [], [], []  # Qtext_id,Qtext_idx,begin,end
            for i in idxs:
                d = self.data[i]
                text, Q = d[0], d[1]
                text = text[:maxlen - len(Q) - 3]
...
def extract_entity(text_in, c_in):  # todo
...
for i, (train_fold, test_fold) in enumerate(kf):
    ...
    model, train_model = modify_bert_model()
    ...
    if True or not os.path.exists(model_path):
        ...
        train_model.fit_generator(train_D.__iter__(),
                                  ...
                                  epochs=10,
                                  callbacks=[evaluator, EarlyStopping(monitor='val_loss', patience=4)],
