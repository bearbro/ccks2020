...
model_save_path = "./data/ccks2020_ckt_256_crossweight_MyGRU_crf/"
...
for split_idx in range(split_nums):
    ...
    for i, (train_fold, test_fold) in enumerate(kf):
        ...
        val_result_path = os.path.join(model_cv_path, str(split_idx) + '_' + "val_result_k" + str(i) + ".pkl")
...
for i, (train_fold, test_fold) in enumerate(kf):
    ...
    model_path = os.path.join(model_save_path, 'finall_' +
                              "modify_bert_biMyGRU_crf_model" + str(i) + ".weights")
...
for i, (train_fold, test_fold) in enumerate(kf):
    ...
    model = modify_bert_model_biMyGRU_crf()
    model_path = os.path.join(model_save_path, 'final' +
                              '_' + "modify_bert_biMyGRU_crf_model" + str(i) + ".weights")
    print("load best model weights ...")
    model.load_weights(model_path)
...
result_path = os.path.join(model_save_path, "final_" + "result_k" + 'cv' + ".txt")
